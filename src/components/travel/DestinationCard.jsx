    ...
    >
      ...
      <div className="relative h-48">
        ...
        <div className="absolute inset-0 p-4 flex flex-col justify-end">
          <h3 className="text-xl font-bold text-rez-navy dark:text-white">{destination.name}</h3>

          <div className="flex items-center gap-1 text-rez-gray-700 dark:text-gray-300 mt-1">
          ...
          <div className="flex items-center gap-3 mt-2">
            <div className="flex items-center gap-1">
              <Plane className="w-3 h-3 text-rez-gray-600 dark:text-gray-400" />
              <span className="text-xs text-rez-gray-700 dark:text-gray-300">₹{destination.flightsFrom.toLocaleString()}</span>
            </div>
            <div className="flex items-center gap-1">
              <Building2 className="w-3 h-3 text-rez-gray-600 dark:text-gray-400" />
              <span className="text-xs text-rez-gray-700 dark:text-gray-300">₹{destination.hotelsFrom.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 bg-white dark:bg-[#2C2C2E] flex items-center justify-between">
        <div className="flex gap-2">
          {destination.highlights.slice(0, 2).map((h) => (
            <span key={h} className="text-[10px] text-rez-gray-600 dark:text-gray-400">{h}</span>
          ))}
        </div>
