...
const MarketCard = ({ market, variant = 'default' }) => {
      ...
      >
        ...
        <div className="relative p-5 min-h-[200px] flex flex-col justify-end">
          ...
          {isLive && market.visitors > 0 && (
            <div className="absolute top-4 right-4 flex items-center gap-1.5 px-2 py-1 rounded-full bg-black/50 backdrop-blur-sm">
              <Users className="w-3 h-3 text-emerald-400" />
              <span className="text-xs text-white">{market.visitors}</span>
            </div>
          )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10">
                ...
                <span className="text-xs text-white">{market.stallCount} stalls</span>
              </div>
  ...
  return (
    <Link
      to={`/flea-market/${market.id}`}
      className="block p-4 rounded-2xl bg-[#2C2C2E] hover:bg-[#3C3C3E] transition-colors"
    >
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h3 className="font-semibold text-white truncate">{market.name}</h3>
            {isLive && (
