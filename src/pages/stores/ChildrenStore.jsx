    ...
    <div className="min-h-screen bg-black pb-24">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-white">Children Store</h1>
      ...
      <div className="mt-6 px-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-semibold text-white">Shop by Category</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          {childrenStore.categories.map((category) => (
            ...
            >
              ...
              <p className="text-xs text-gray-400 mt-1">{category.items} products</p>
              <p className="text-xs text-gray-500">Ages: {category.ageRange}</p>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl bg-[#2C2C2E]">
        <h3 className="font-semibold text-white mb-3">Why Shop Children's on ReZ?</h3>
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">🛡️</span>
            <h4 className="text-sm font-medium text-white mt-1">Safety First</h4>
            <p className="text-xs text-gray-400 mt-0.5">All products verified</p>
          </div>
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">👨‍👩‍👧</span>
            <h4 className="text-sm font-medium text-white mt-1">Family Wallet</h4>
            <p className="text-xs text-gray-400 mt-0.5">Share coins with family</p>
          </div>
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">🎁</span>
            <h4 className="text-sm font-medium text-white mt-1">Smart Bundles</h4>
            <p className="text-xs text-gray-400 mt-0.5">Save on combos</p>
          </div>
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">💰</span>
            <h4 className="text-sm font-medium text-white mt-1">High Cashback</h4>
            <p className="text-xs text-gray-400 mt-0.5">Up to 18% back</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between px-4 mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-white">Safe & Fun Products</h2>
          </div>
          <button className="text-xs flex items-center gap-1" style={{ color: theme.primary }}>
            View All <ChevronRight className="w-3 h-3" />
          </button>
        ...
        <div className="flex gap-4 px-4 overflow-x-auto hide-scrollbar pb-2">
          {childrenStore.products.map((product) => (
            <Link
              ...
              className="min-w-[180px] p-4 rounded-2xl bg-[#2C2C2E] shrink-0 active:scale-[0.98] transition-transform relative"
            >
              {product.safetyBadge && (
              ...
              <p className="text-xs text-gray-400">Ages: {product.ageRange}</p>
      ...
      <div className="mt-6 px-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="font-semibold text-white">Special Bundles</h2>
        </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">{bundle.items} items included</p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-sm line-through text-gray-500">₹{bundle.originalPrice.toLocaleString()}</span>
                    <span className="text-lg font-bold text-white">₹{bundle.price.toLocaleString()}</span>
                  </div>
                </div>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl" style={{ backgroundColor: `${theme.primary}20` }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-medium text-white">Family Wallet</h3>
        </div>
        <p className="text-sm text-gray-400 mb-3">{childrenStore.familyWallet.description}</p>
        <p className="text-xs text-gray-500 mb-3">Up to {childrenStore.familyWallet.maxMembers} members</p>
        <button className="w-full py-2.5 rounded-xl text-sm font-medium text-white" style={{ backgroundColor: theme.primary }}>
          Setup Family Wallet
        </button>
      </div>
          ...
          {['FirstCry', 'Mothercare', 'Hamleys'].map((store, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-[#2C2C2E] flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">{1.5 + idx * 0.7} km away</p>
              </div>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl border" style={{ backgroundColor: `${theme.primary}10`, borderColor: `${theme.primary}30` }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="font-semibold text-white">Super Parent Progress</h3>
            <p className="text-xs text-gray-400">3 more purchases to unlock bonus</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs text-gray-400">2/5 purchases</span>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl bg-[#2C2C2E] text-center">
        <h3 className="font-semibold text-white mb-2">Joy for the Little Ones.</h3>
        <p className="text-sm text-gray-400">Safe products. Happy families. With ReZ rewards.</p>
      </div>
      ...
      <div className="fixed bottom-20 left-0 right-0 px-4 z-50">
        <div className="p-3 rounded-2xl shadow-xl flex items-center justify-between" style={{ backgroundColor: theme.primary }}>
          <div>
            <p className="text-sm font-medium text-white">👶 Save ₹1,500 on bundles</p>
            <p className="text-xs text-white/70">Safe & verified products</p>
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" className="bg-white/20 text-white border-0">Bundles</Button>
