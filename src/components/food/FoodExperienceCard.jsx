...
const FoodExperienceCard = ({ experience }) => {
  ...
  return (
    <div className="min-w-[300px] rounded-2xl overflow-hidden bg-[#2C2C2E] shrink-0 group">
      {/* Image */}
      <div className="relative h-40">
        ...
        <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-1 rounded-full bg-black/60 backdrop-blur-sm">
          ...
          <span className="text-xs text-white font-medium">{experience.rating}</span>
        </div>

        {/* Title */}
        <div className="absolute bottom-3 left-3 right-3">
          <h3 className="font-semibold text-white">{experience.title}</h3>
          <p className="text-sm text-gray-300">{experience.subtitle}</p>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs text-gray-400">{experience.time}</span>
        </div>

        {/* Includes */}
        <div className="mb-3">
          <p className="text-[10px] text-gray-500 mb-1">INCLUDES</p>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-white/5">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold text-white">₹{experience.price}</span>
