...
const FashionBrandCard = ({ brand }) => {
  return (
    <Link
      ...
      className="min-w-[100px] flex flex-col items-center gap-2 p-4 rounded-2xl bg-[#2C2C2E] shrink-0 active:scale-95 transition-transform relative"
    >
      {/* Tag */}
      {brand.tag && (
        <span className="absolute top-2 right-2 px-1.5 py-0.5 rounded-full bg-blue-500 text-[8px] text-white">
          {brand.tag}
        </span>
      )}
