...
const ModeSwitcher = () => {
  ...
  const modes = [
    { id: 'nearYou', label: 'ReZ Near You', icon: MapPin, description: 'Local stores, offline + delivery' },
    { id: 'mall', label: 'ReZ Mall', icon: ShoppingBag, description: 'Curated brands & collections' },
    { id: 'cashStore', label: 'Cash Store', icon: Wallet, description: 'Affiliate shopping + coupons' },
    { id: 'prive', label: 'Privé', icon: Crown, description: 'Invite-only exclusive deals', locked: false },
  ];
      ...
      <div className="flex gap-1 p-1 rounded-2xl bg-rez-gray-100 dark:bg-[#1C1C1E]">
        {modes.map((mode) => {
          ...
          return (
            <button
              key={mode.id}
              onClick={() => !mode.locked && setGlobalMode(mode.id)}
