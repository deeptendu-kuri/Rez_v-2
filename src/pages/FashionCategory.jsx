...
const priceRanges = [
  ...
  { id: 'under1k', label: 'Under ₹1,000', min: 0, max: 1000 },
  ...
  { id: 'above10k', label: 'Above ₹10,000', min: 10000, max: Infinity },
];
...
const FashionCategory = () => {
  ...
  // Get category info
  const categoryInfo = fashionCategories.find((c) => c.id === category);

  // Filter products by category
  let filteredProducts = fashionProducts.filter((p) => p.category === category);
  ...
  if (activeFilters.includes('trending')) {
    filteredProducts = filteredProducts.filter((p) => p.tag === 'Trending' || p.tag === 'Bestseller');
  }
      ...
      <div className="sticky top-0 z-40 glass">
        ...
        <div className="px-4 pb-3">
          <div className="flex gap-2 overflow-x-auto hide-scrollbar">
            {fashionFilters.slice(0, 6).map((filter) => (
              <button
                key={filter.id}
        ...
        <div className="px-4 mb-4">
          <div className="flex flex-wrap gap-2">
            ...
            {activeFilters.map((filterId) => {
              const filter = fashionFilters.find((f) => f.id === filterId);
              return (
                <span
                  key={filterId}
