...
const FinancialTransactionCard = ({ transactions }) => {
  return (
    <div className="p-4 rounded-2xl bg-white dark:bg-[#2C2C2E]">
      <h3 className="font-semibold text-rez-navy dark:text-white mb-3">Recent Transactions</h3>
        ...
        {transactions.map((txn) => (
          <div key={txn.id} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-rez-navy dark:text-white">{txn.type}</p>
              <p className="text-xs text-rez-gray-600 dark:text-gray-400">{txn.date}</p>
            </div>
            <div className="text-right">
              <p className="text-sm font-medium text-rez-navy dark:text-white">₹{txn.amount}</p>
