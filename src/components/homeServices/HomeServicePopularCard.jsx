...
const HomeServicePopularCard = ({ service }) => {
  return (
    ...
    >
      <h3 className="font-medium text-rez-navy dark:text-white mb-1">{service.name}</h3>

      <div className="flex items-center gap-1 mb-2">
        <TrendingUp className="w-3 h-3 text-emerald-400" />
        <span className="text-[10px] text-rez-gray-600 dark:text-gray-400">{service.bookings}</span>
      </div>

      <div className="flex items-center justify-between">
        <div>
          <p className="text-lg font-bold text-rez-navy dark:text-white">₹{service.price}</p>
