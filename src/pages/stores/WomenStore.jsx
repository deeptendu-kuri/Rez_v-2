  ...
  return (
    <div className="min-h-screen bg-black pb-24">
        ...
        <div className="px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-white">Women Store</h1>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl bg-[#2C2C2E]">
        <h3 className="font-semibold text-white mb-3">Why Shop Women's on ReZ?</h3>
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">💖</span>
            <h4 className="text-sm font-medium text-white mt-1">Curated for You</h4>
            <p className="text-xs text-gray-400 mt-0.5">Personalized picks</p>
          </div>
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">💰</span>
          ...
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">🎂</span>
            <h4 className="text-sm font-medium text-white mt-1">Birthday Bonus</h4>
            <p className="text-xs text-gray-400 mt-0.5">2x coins on your day</p>
          </div>
          <div className="p-3 rounded-xl bg-white/5">
            <span className="text-xl">👭</span>
            <h4 className="text-sm font-medium text-white mt-1">Community</h4>
            <p className="text-xs text-gray-400 mt-0.5">Real reviews</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 px-4">
        ...
        <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-2">
          {womenStore.occasions.map((occasion) => (
            ...
            >
              <h3 className="font-medium text-white mb-1">{occasion.name}</h3>
              <p className="text-sm text-gray-400">{occasion.offers} offers</p>
              <div className="flex items-center gap-1 mt-2">
                <Coins className="w-3 h-3" style={{ color: theme.primary }} />
      ...
      <div className="mt-6 px-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="font-semibold text-white">Creator Picks</h2>
        </div>
        <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-2">
          {womenStore.creatorReels.map((reel) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80">
                <p className="text-xs text-white font-medium">{reel.creator}</p>
                <p className="text-[10px] text-gray-400">{reel.views} views</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="mt-6 px-4">
        ...
        <div className="space-y-3">
          {womenStore.communityReviews.map((review) => (
            <div key={review.id} className="p-4 rounded-2xl bg-[#2C2C2E]">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-lg">👩</span>
                <div>
                  <p className="text-sm font-medium text-white">{review.user}</p>
                  <p className="text-xs text-gray-400">{review.bodyType || review.concern}</p>
                </div>
              ...
              <p className="text-xs text-gray-500 mt-1">on {review.product}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          {['Zara', 'H&M', 'Sephora'].map((store, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-[#2C2C2E] flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">{1.2 + idx * 0.6} km away</p>
              </div>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl border" style={{ backgroundColor: `${theme.primary}10`, borderColor: `${theme.primary}30` }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="font-semibold text-white">Style Queen Progress</h3>
      ...
      <div className="mx-4 mt-6 p-4 rounded-2xl bg-[#2C2C2E] text-center">
        <h3 className="font-semibold text-white mb-2">Empowerment Through Choice.</h3>
        <p className="text-sm text-gray-400">Your style. Your rewards. With ReZ.</p>
      </div>
      ...
      <div className="fixed bottom-20 left-0 right-0 px-4 z-50">
        <div className="p-3 rounded-2xl shadow-xl flex items-center justify-between" style={{ backgroundColor: theme.primary }}>
          <div>
            <p className="text-sm font-medium text-white">💖 Earn ₹2,500 in rewards</p>
            <p className="text-xs text-white/70">on Women's favorites today</p>
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" className="bg-white/20 text-white border-0">Occasions</Button>
